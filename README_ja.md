...
誤解を恐れず言うと、本コーディング環境はBootstrapによりリセットし、それをBEMコーディンでマウントするスタイルです。
...
別コンポーネントを定義しても良さそうですが、ここでは同一コンポーネント内でのBEMでの拡張を徹底してみましょう。
...
またこのモディファイアは.btnを継承した .btn-primary, .btn-secondary...クラスと同等の「継承」を意味するものと考えます。
