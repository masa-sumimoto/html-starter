...
※ もし、有名かつより良いスターターキットをお探しならば [web starter kid of google](https://github.com/google/web-starter-kit) をオススメします。  
※ あくまで、一般的なWEBページを作成することを想定しています。ReactやVueを用いたWebアプリと呼ばれるもののコーディングを想定する場合はまた別のアイデアの方が有効です。（いつかまとめてドキュメントにまとめたいと思っています）  
...
## コーディングスタイル概要
私はBootstrapのコンポーネントをベースとし、それを拡張する方法を基本にコーディングを行っています。
しかしOOCSS設計に基づくBootstrap4のスタイルに反し、新規で追加するスタイルにはBEM設計を用いるようにしています。
これらを異なるコンセプトの差異は意外に干渉し合いません。
理由の一つにページ単位、パーツ（コンポーネント）単位にスタイルを切り分けることで、1スタイルシートが担うコード量が
劇的に削減できることがあげられると思います。

常に見通しの良いスタイル管理を行い、Bootstrapと、うまくやっていく方法を探ってゆきましょう！
...
注意したいことは、これらは「自身のレイアウトデザイン」及び「内包する要素間の共通のデザインレギュレーション」を定義することが主な役割だという点です。
...
また、これら最上位のレイアウトブロックに用いるクラス名は最初の文字だけアッパーケースを利用することで、
その他要素との違いをつけています。
...
```
あなたがWEBサイトを制作した。
ローンチ後、クライアント先のWEB担当者がサイトをメンテすることになった。
彼の日常業務は非常に軽微なデザイン（スタイル）の修正や更新である。
大規模な改修業務修正の際には改めて改修作業をあんたが請け負う。
```
